...
import { ChevronDown, ChevronUp, ExternalLink, TrendingUp, Search, Users, Eye, Zap, ShieldCheck, Activity, DollarSign, Database, Cpu, Globe, BrainCircuit } from 'lucide-react';
          ...
          {provider.models && provider.models.length > 0 && (
            <section className="space-y-10">
              <div className="flex items-center gap-5 text-slate-100 uppercase tracking-[0.3em] font-black text-xs">
                <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-indigo-400"><BrainCircuit size={20}/></div>
                Engine Specifications
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {provider.models.map((m, i) => (
                  <div key={i} className="p-6 rounded-3xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.05] transition-all group/mod">
                    <div className="flex justify-between items-start mb-4">
                      <h4 className="text-lg font-bold text-white group-hover/mod:text-indigo-300 transition-colors">{m.name}</h4>
                      {m.reasoning && (
                        <span className="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-400 text-[8px] font-black tracking-widest uppercase border border-indigo-500/20">Reasoning</span>
                      )}
                    </div>
                    <div className="space-y-3 text-xs font-mono">
                      <div className="flex flex-col gap-1">
                        <span className="text-slate-600 uppercase text-[8px] tracking-widest">Modality</span>
                        <span className="text-slate-400">{m.modality}</span>
                      </div>
                      {m.context && (
                        <div className="flex flex-col gap-1">
                          <span className="text-slate-600 uppercase text-[8px] tracking-widest">Context</span>
                          <span className="text-slate-200">{m.context}</span>
                        </div>
                      )}
                      {m.strength && (
                        <div className="flex flex-col gap-1">
                          <span className="text-slate-600 uppercase text-[8px] tracking-widest">Strength</span>
                          <span className="text-slate-400 italic">{m.strength}</span>
                        </div>
                      )}
                      {m.behavior && (
                        <div className="flex flex-col gap-1">
                          <span className="text-slate-600 uppercase text-[8px] tracking-widest">Behavior</span>
                          <span className="text-amber-400/80">{m.behavior}</span>
                        </div>
                      )}
                    </div>
                  </div>
